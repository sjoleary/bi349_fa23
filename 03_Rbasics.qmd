...
The `assignment operator` (`<-`) assigns the `value` on the right of the `<-` to the `object` on the left^[Start building good habits starting now in terms of your coding sytle. For example, your code is a lot more readable if you use whitespace to your advantage. For example, make sure you have a space before and after your ` <- `].
...
## {{< fa clipboard-question >}}   Give it a try.
...
## {{< fa clipboard-question >}}   Let's give this a whirrl.
